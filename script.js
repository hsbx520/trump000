...
const RATES = {
    // 1 ETH = 435,600 WLFG
    'eth': { wlfg_per_unit: 435600, min: 0.1 },
    // 1 BNB = 78,440 WLFG
    'bnb': { wlfg_per_unit: 78440, min: 0.3 },
    // 1 USDT = 88 WLFG
...
document.getElementById('buy-btn').addEventListener('click', function() {
    ...
    const networkName = currency === 'bnb' ? 'Binance Smart Chain' : 'Ethereum mainnet';
    ...
    document.getElementById('modal-network-name').textContent = networkName;
...
function startCountdown() {
    // Target date: August 25, 2025, 13:00 PM US Eastern Time (EDT which is UTC-4)
    const targetDate = new Date('2025-08-25T12:00:00-05:00').getTime();
...
class PurchaseNotificationSystem {
    ...
    randomAmount() {
        const currencies = ['ETH', 'BNB', 'USDT', 'USDC'];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Hide the irrelevant FAQ item for USD1
    const faqItemUsd1 = document.getElementById('faq-item-usd1');
    if (faqItemUsd1) {
        // Instead of removing, we hide it to prevent breaking any scripts if they reference it
        faqItemUsd1.style.display = 'none';
    }
